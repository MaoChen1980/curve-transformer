...
**E — 显式状态输入（收官实验）**：GRUCell 的输入从 `xn` 改为 `cat(xn, h_proj(h))`，让门控显式看到 h。参数增加：~787K。结果：fwd loss 略降但 step loss 反而更差，说明 GRU 内部的 W_hh 已提供了足够的 h 信息，额外拼接冗余。

**训练设置**：9 个变体（base + A + B + C + D + AC + ABC + ABCD），纯唐诗 from scratch，base 训练 5000 步，其余 2000 步（变体之间已经可对比）。
...
- **E（显式 h 输入）无效** — 收官实验确认 GRU 内部 W_hh 已提供充分的状态信息给门控，瓶颈不在"能否看到 h"
...
**E — Explicit h in GRU input (final experiment)**: Changed GRUCell input from `xn` to `cat(xn, h_proj(h))`, giving the gate direct visibility of h. Parameter delta: +787K. Result: fwd loss improved slightly but step loss got worse, confirming GRU's internal W_hh already provides sufficient h information — extra concatenation is redundant.

**Training**: 9 variants (base + A + B + C + D + E + AC + ABC + ABCD), pure Tang poetry from scratch, base trained 5000 steps, others 2000 steps.
...
2000  | ABC     | 4.88     | 5.49      | 5.41M
2000  | ABCD    | 5.10     | 5.18      | 6.60M
2000  | E       | 4.38     | 6.49      | 5.41M  ← final experiment
```
...
- **E (explicit h input) ineffective** — GRU's internal W_hh already provides sufficient state info to gates
