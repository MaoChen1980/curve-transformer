"""
Train v4 - D2: Joint forward + step training with independent gp/sp.
No param freezing. Both paths optimized simultaneously.
"""
...
CKPT = "E:/claude/myllm/checkpoint_v4_joint.pt"
...
# Small init for gp/sp so step path doesn't NaN on early steps
for name, p in model.named_parameters():
    if any(x in name for x in ('.gp.', '.sp.')):
        if 'weight' in name: nn.init.normal_(p, mean=0.0, std=0.01)
...
if os.path.exists(CAUSAL_CKPT):
    ...
    print(f"Loaded causal checkpoint. New step params (gp/sp): {len(unexp)}", flush=True)
    print(f"  {unexp}", flush=True)
else:
    print("No causal checkpoint! Run train_v4_causal.py first.", flush=True); sys.exit(1)

# All params trainable (no freezing)
trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
total = sum(p.numel() for p in model.parameters())
print(f"Trainable: {trainable:,} / {total:,} (all)", flush=True)
...
torch.save({"model_state": model.state_dict(), "c2i": c2i,
            "i2c": {v:k for k,v in c2i.items()}}, "E:/claude/myllm/model_v4_joint.pt")
print("Model saved -> model_v4_joint.pt", flush=True)
