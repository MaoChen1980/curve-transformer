"""
Test poetry D3: GRUCell step model (Tang poetry only). Tests BOTH forward() and step().
"""
import sys, os, math
...
CKPT = "E:/claude/myllm/model_v4_poetry_d3.pt"
if not os.path.exists(CKPT):
    print("No poetry D3 model found!")
    sys.exit(1)
...
model.eval()
print(f"Loaded poetry D3 model | Device: {device} | Params: {sum(p.numel() for p in model.parameters()):,}")
...
prompts = ["今天天气","春风吹","明月","静夜思","春晓","登高","望月","朝辞","床前","白日依山"]
print("\n" + "="*60)
print("Poetry D3 — GRUCell step model (poetry only)")
print("="*60)
print("\n--- forward() path ---")
for p in prompts:
    out = gen_forward(p)
