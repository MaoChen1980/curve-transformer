"""
Train poetry D2: Joint forward + step training, Tang poetry only.
Independent gp/sp, no param freezing, train from scratch.
"""
...
CKPT = "E:/claude/myllm/checkpoint_v4_poetry_joint.pt"
...
class Block(nn.Module):
    def __init__(self):
        ...
        nn.init.normal_(self.gp.weight, 0, 0.01); nn.init.zeros_(self.gp.bias)
        nn.init.normal_(self.sp.weight, 0, 0.01); nn.init.zeros_(self.sp.bias)
    def forward(self,x):
...
torch.save({"model_state": model.state_dict(), "c2i": c2i,
            "i2c": {v:k for k,v in c2i.items()}}, "E:/claude/myllm/model_v4_poetry_joint.pt")
print("Model saved -> model_v4_poetry_joint.pt", flush=True)
