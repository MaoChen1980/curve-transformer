"""
Test poetry D2: Joint model (Tang poetry only). Tests BOTH forward() and step().
"""
import sys, os, math
...
CKPT = "E:/claude/myllm/model_v4_poetry_joint.pt"
if not os.path.exists(CKPT):
    print("No poetry joint model found!")
    sys.exit(1)
...
model.eval()
print(f"Loaded poetry Joint model | Device: {device} | Params: {sum(p.numel() for p in model.parameters()):,}")
...
print("\n" + "="*60)
print("Poetry D2 — Joint model (poetry only)")
print("="*60)
print("\n--- forward() path ---")
for p in prompts:
    out = gen_forward(p)
